        ...
        node.deprecationSummaryVariants = VariantCollection(
            ...
            transform: { trait in
                if traitsInWhichSymbolsIsDeprecated.contains(trait) || traitsInWhichSymbolsIsDeprecated.isEmpty {
                    ...
                    //
                    // Also, previous versions of DocC treated a page as deprecated if it had a custom `@DeprecationSummmary`
                    // description, even if the symbol wasn't deprecated. We preserve that behavior for backwards compatibility.
